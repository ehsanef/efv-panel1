...
export async function getConfigAddresses(domain: string, isFragment: boolean): Promise<string[]> {
    ...
    /* EFV: tolerate DoH failures (blocked resolver / offline) — fall back to domain + clean IPs. */
    let ipv4: string[] = [];
    let ipv6: string[] = [];
    try {
        ({ ipv4, ipv6 } = await resolveDNS(domain, !enableIPv6));
    } catch (e) {
        console.log(`DNS resolve failed for ${domain}, using domain address only:`, e);
    }
